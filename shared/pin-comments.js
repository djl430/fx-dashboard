...
(function () {
    ...
    function firstIdentityChar(value) {
        const text = normalizeNick(value);
        const match = text.match(/[\p{Script=Han}A-Za-z0-9]/u);
        return match ? match[0] : '';
    }

    function pinLabel(pin, fallbackNumber) {
        if (PIN_LABEL_MODE === 'author-initial') {
            const firstCommentAuthor = pin.comments && pin.comments[0] && pin.comments[0].author;
            const label = firstIdentityChar(firstCommentAuthor || pin.createdBy);
    ...
    function setNick(n) {
        localStorage.setItem(NICK_KEY, normalizeNick(n));
    }
    function getView() {
    ...
    function renderPins() {
        ...
        if (!pinsHidden) {
            ...
            local.forEach((p, i) => {
                ...
                pinEl.innerHTML = `<span>${pinLabel(p, i + 1)}</span>`;
    ...
    function renderCard(card, pin) {
        ...
        const numLabel = idx >= 0 ? `#${pinLabel(pin, idx + 1)}` : '#?';
        card.innerHTML = `
            <div class="pc-card-head">
    ...
    function renderDrawerList() {
        ...
        list.innerHTML = view.map(({ p, num }) => {
            ...
            const numText = pinLabel(p, num != null ? num : '·');
    ...
    function init() {
        ...
        window.addEventListener('review:canvas-transform', reposition);
